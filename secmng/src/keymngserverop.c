...
int MngServer_Agree(MngServer_Info *svrInfo, MsgKey_Req *msgkeyReq, unsigned char **outData, int *datalen)
{
	...
	//获取keysn
	ret = KeyMngsvr_DBOp_GenKeyID(handle, &tmpkeyid);//数据裤里会自动++
	if (ret != 0) 
	{
		KeyMng_Log(__FILE__, __LINE__,KeyMngLevel[4], ret,"func IC_DBApi_ConnGet() err");
		goto End;
...
int MngServer_Check(MngServer_Info *svrInfo, MsgKey_Req *msgkeyReq, unsigned char **outData, int *datalen)
{
	...
	if ( memcmp(nodeShmInfo.seckey, msgkeyReq->r1, 8 ) == 0&&nodeShmInfo.status==0)
	{
		msgKeyRes.rv  = 0;
	}
	else
...
{//服务器各种信息 解析好的客户端请求报文 待传出数据
	...
	if (ret != 0) 
	{
		...
		sprintf(mySql,"update SECKYEINFO set state=1 where KEYID=%d",nodeInfo.seckeyid);
	...
	//组织应答报文 MsgKey_Res rv=0
	msgKeyRes.rv  = 0;
	strcpy(msgKeyRes.clientId, msgkeyReq->clientId);
	strcpy(msgKeyRes.serverId, svrInfo->serverId);	
	memcpy( msgKeyRes.r2, nodeInfo.seckey, 8); 
	msgKeyRes.seckeyid =  nodeInfo.seckeyid;

	if ( memcmp(nodeInfo.seckey, msgkeyReq->r1, 8 ) == 0)
	{
		msgKeyRes.rv  = 0;
	}
	else
	...
	/*
	//写数据库
	ret = KeyMngsvr_DBOp_WriteSecKey(handle, &nodeInfo) ;
	if (ret != 0)
	{
		KeyMng_Log(__FILE__, __LINE__,KeyMngLevel[4], ret,"func KeyMngsvr_DBOp_WriteSecKey() err");
		goto End;
